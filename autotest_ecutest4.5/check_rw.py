    ...
    def data_repalce(self,blockname,m,name_temp,data_val_list,map_exl_enum_dict,flag_chk,listmessagebox,flgerror):
        ...
        for n in range(len(data_val_list)):
            ...
            if data_val.strip()=='':
                ...
                if '||' in data_val or '&&' in data_val:
                    data_val = data_val.replace('||',' or ')
                    data_val = data_val.replace('&&',' and ')
                    pattern = 'v[><=]=*'#re expression:check v> v>= v< v<= v==
                    position = [p.end() for p in re.finditer(pattern,data_val)]
                    ...
                    for k in position:
                        ...
                        for q in range(k,length):
                            if data_val[q] != ' ':
                                val_temp= val_temp+data_val[q]
                            else:
                                break
                        if not self.is_number(val_temp):
                            val_temp_list.append(val_temp)
                            
                    for val_temp in val_temp_list:
                        pat = 's\d'# re expression :check have s0 or not
                        pa = re.compile(pat)
                        val_templ=re.findall(pa,val_temp)
                        if val_templ ==[]:
                            if val_temp in map_exl_enum_dict.keys():
                                val_temp_rep = str(map_exl_enum_dict[val_temp])
                                data_val= data_val.replace(val_temp,val_temp_rep)                            
                            ...
                            pass
                    data_val=data_val.replace('v','value')
                    
                    data_val_list[n][m]=data_val
                else:
                    if not self.is_number(data_val):
                        ...
                        if position == []:
                            if data_val in map_exl_enum_dict.keys():
                                ...
                                if flag_chk:
                                    data_val_list[n][m]='value=='+data_val
                                else:
                                    data_val_list[n][m]=data_val
                            elif 's' in data_val: #use ''s' in val_temp' is not right,but can run (s0,s1)
                            ...
                            if val_temp in map_exl_enum_dict.keys():
                                ...
                                data_val_list[n][m]=data_val
                            elif self.is_number(val_temp) or 's' in val_temp:#use ''s' in val_temp' is not right,but can run (s0,s1,sw)
                                data_val=data_val.replace('v','value')
                                data_val_list[n][m]=data_val
                            else:
                                print 'Case<%s> Block<%s> Colm%d Row %d donot have right value'%(name_temp,blockname,m,n)
...
##    os.chdir(oldpath)
    ...
    HILname = 'DSPACE'
    usermapfpath = 'E:\\testcase\\case_lib\\new\\map\\usermap.xls'
    modelmapfpath = 'E:\\temp\\py\\GUI\\autotest_ecutest4.5\\modelmap_dspace.txt'
    a2lpath = 'E:\\temp\\py\\GUI\\autotest_ecutest4.5\\KL0104AC00-L39355.a2l'
