...
class Check_rw:
    ...
    def list_postion(self,signal,dbc_info):
        #try:
        flag_findsig=0
        pos_findsig=[]
        if flag_findsig==0:
            list_temp=dbc_info.Chassisbus.getinfo.signallist
            #print list_temp
            for n in range(len(list_temp)):
                #try:
                if signal in list_temp[n]:
                    pos_temp=list_temp[n].index(signal)
                    pos_findsig=['Chassis',n,pos_temp]
                    flag_findsig=1
                    break
                else:
                    flag_findsig=0
        ...
        if flag_findsig==0:
            list_temp=dbc_info.Hybridbus.getinfo.signallist
            for n in range(len(list_temp)):
                #try:
                if signal in list_temp[n]:
                    pos_temp=list_temp[n].index(signal)
                    pos_findsig=['Hybrid',n,pos_temp]
                    flag_findsig=1
                    break
                else:
                    flag_findsig=0
        ...
        if flag_findsig==0:
            list_temp=dbc_info.PTbus.getinfo.signallist
            for n in range(len(list_temp)):
                #try:
                if signal in list_temp[n]:
                    pos_temp=list_temp[n].index(signal)
                    pos_findsig= ['PT',n,pos_temp]
                    flag_findsig=1
                    break
                else:
                    flag_findsig=0
        ...
        if flag_findsig==0:
            list_temp=dbc_info.LINbus.getinfo.signallist
            for n in range(len(list_temp)):
                #try:
                if signal in list_temp[n]:
                    pos_temp=list_temp[n].index(signal)
                    pos_findsig= ['LIN',n,pos_temp]
                    flag_findsig=1
                    break
                else:
                    flag_findsig=0
...
##                    data_sig_final='Plant model/'+data_sig_temp
    ...
    def signal_check(self,blockname,m,name_temp,data_val_list,data_sig,map_exlin,map_exlout,map_dev,can,flag_chk,HILname,
                     listmessagebox,flgerror):

        sig_dbcpos_info=self.list_postion(data_sig,can)#signal in DBC postion
        ...
        if flag_chk==True:
            ...
            if data_sig in map_exlout.keys():#find signal in map_excel
                ...
                if map_exlout_type=='IO': #find signal type and name in model
                ...
                elif map_exlout_type=='CAN': #find signal type and name in model
                    dbc_pos_info=self.list_postion(map_exlout_name,can)
                    if dbc_pos_info:
                        if dbc_pos_info[0]=='Chassis':
                            can_type='Chassis'
                            message_name=can.Chassisbus.getinfo.messagename[dbc_pos_info[1]]
                            ...
                            if reflag == False:
                                return False,None,None
                            #return True data_sig_list_temp,data_val_list_temp         
                        elif dbc_pos_info[0]=='Hybrid':
                            can_type='Hybrid'
                            message_name=can.Hybridbus.getinfo.messagename[dbc_pos_info[1]]
                            reflag,data_sig_list_temp,data_val_list_temp=self.can_check(HILname,modelname_ETAS,modelname_DSPACE,CAN_name,message_name,
                                                                                 map_exlout_name,map_dev,can_type,blockname,m,name_temp,data_val_list,map_exlout_enum_dict,flag_chk,listmessagebox,flgerror)
                            ...
                            if reflag == False:
                                return False,None,None
                            #return True data_sig_list_temp,data_val_list_temp         
                        elif dbc_pos_info[0]=='PT':
                            can_type='PT'
                            message_name=can.PTbus.getinfo.messagename[dbc_pos_info[1]]
                            reflag,data_sig_list_temp,data_val_list_temp=self.can_check(HILname,modelname_ETAS,modelname_DSPACE,CAN_name,message_name,
                                                                                 map_exlout_name,map_dev,can_type,blockname,m,name_temp,data_val_list,map_exlout_enum_dict,flag_chk,listmessagebox,flgerror)
                            if reflag == False:
                                return False,None,None
                            #eturn True data_sig_list_temp,data_val_list_temp 
                        else:
                            print map_exl_name+' cannot find in dbc'
                            messagebox = map_exl_name+' cannot find in dbc'
                            listmessagebox.append(messagebox)
                        ...
                        messagebox='Signal<'+str(map_exl_name)+'> of Case<'+str(name_temp)+'> cannot find in dbc'
                        listmessagebox.append(messagebox)
                ...
                elif map_exlout_type=='LIN':

                    dbc_pos_info=self.list_postion(map_exlout_name,can)
                    if dbc_pos_info:
                        if dbc_pos_info[0]=='LIN':
                            can_type='LIN'
                            message_name=can.LINbus.getinfo.messagename[dbc_pos_info[1]]
                            ...
                            print 'Signal<%s> of Case<%s> cannot find in ldf'%(map_exl_name,name_temp)
                            #ui_MainCreate.Ui_MainCreateMw.textBrowser.append('aaaaaaaa')

                            messagebox='Signal<'+str(map_exl_name)+'> of Case<'+str(name_temp)+'> cannot find in ldf'
                            listmessagebox.append(messagebox)
                    ...
                    print 'Signal<%s> of Case<%s> have  wrong type'%(map_exl_name,name_temp)
                            #ui_MainCreate.Ui_MainCreateMw.textBrowser.append('aaaaaaaa')

                    messagebox='Signal<'+str(map_exl_name)+'> of Case<'+str(name_temp)+'> have  wrong type'
                    listmessagebox.append(messagebox)
            ...
            if data_sig in map_exlin.keys():#find signal in map_excel
                ...
                if map_exlin_type=='IO': #find signal type and name in model
                ...
                elif map_exlin_type=='CAN': #find signal type and name in model
                    dbc_pos_info=self.list_postion(map_exlin_name,can)
                    if dbc_pos_info:
                        if dbc_pos_info[0]=='Chassis':
                            can_type='Chassis'
                            message_name=can.Chassisbus.getinfo.messagename[dbc_pos_info[1]]
                            ...
                            print map_exl_name+' cannot find in dbc'
                            messagebox=str(map_exl_name)+' cannot find in dbc'
                            listmessagebox.append(messagebox)
                    ...
                    if map_exlin_name in can.Chassisbus.getinfo.messagename:
    ...
    ##                    bus_prio_name='CANModule/ChassisCAN/'+canname_temp+'/'+msgID_temp+'std_'+map_exl_name+'/ManualEnabled'
                        if HILname=='ETAS':
                            bus_prio_name = modelname_ETAS+BUS_name+'EnChassis/'+map_exlin_name+'_enable/Value'
                            ...
                            bus_prio_name = modelname_DSPACE+BUS_name+'EnChassis/'+map_exl_name+'_enable/Value'
                    ...
                    elif map_exl_name in can.Hybridbus.getinfo.messagename:
##                    pos_temp=can.Hybridbus.getinfo.messagename.index(map_exl_name)
##                    canname_temp=can.Hybridbus.getinfo.canname[pos_temp]
##                    msgID=can.Hybridbus.getinfo.messageid[pos_temp]
##                    msgID_temp=self.hexid(msgID)
##                    bus_prio_name='CANModule/HybridCAN/'+canname_temp+'/'+msgID_temp+'std_'+map_exl_name+'/ManualEnabled'
                        if HILname=='ETAS':
                            bus_prio_name = modelname_ETAS+BUS_name+'EnHybrid/'+map_exl_name+'_enable/Value'
                            ...
                            bus_prio_name = modelname_DSPACE+BUS_name+'EnHybrid/'+map_exl_name+'_enable/Value'
                        ...
                        #sys.exit()
                    elif map_exl_name in can.PTbus.getinfo.messagename:
    ##                    pos_temp=can.PTbus.getinfo.messagename.index(map_exl_name)
    ##                    canname_temp=can.PTbus.getinfo.canname[pos_temp]
    ##                    msgID=can.PTbus.getinfo.messageid[pos_temp]
    ##                    msgID_temp=self.hexid(msgID)
    ##                    bus_prio_name='CANModule/PTCAN/'+canname_temp+'/'+msgID_temp+'std_'+map_exl_name+'/ManualEnabled'

                        if HILname=='ETAS':
                            bus_prio_name = modelname_ETAS+BUS_name+'EnPT/'+map_exl_name+'_enable/Value'
                            ...
                            bus_prio_name = modelname_DSPACE+BUS_name+'EnPT/'+map_exl_name+'_enable/Value'
                        ...
                        print 'Signal<%s_ManualEnabled> of Case<%s> cannot find in dbc'%(map_exl_name,name_temp)
                        #ui_MainCreate.Ui_MainCreateMw.textBrowser.append('aaaaaaaa')

                        messagebox='Signal<'+str(map_exl_name)+'_ManualEnabled> of Case<'+str(name_temp)+'> cannot find in dbc'
                        listmessagebox.append(messagebox)
    ...
    def check_map(self,case_dict,can,HILname,usermapfpath,modelmapfpath,listmessagebox,flgerror):
            ...
            for i in range(len(predata)):
                ...
                for m in range(len(data_sig_list)):
                    ...
                    if data_sig:
                        ...
                        reflag,data_sig_list_temp,data_val_list_temp=self.signal_check(blockname,m,name_temp,data_val_list,data_sig,map_exlin,
                                                                                map_exlout,map_dev,can,flag_chk,HILname,listmessagebox,flgerror)
...
##    
    ...
    case_dict=check.check_map(case_dict,can)
    #print case_dict
##    print predata
##    print prechk
##    print casedata
